...
public class UserService {
    ...
    public UserDTO updateUser(UUID id, UserUpdateRequest request) {
        ...
        // Guard: don't allow demoting the last remaining ADMIN away from ADMIN - that
        // would lock every admin-only endpoint with no way back in short of a direct
        // database edit.
        if (user.getRole() == Role.ADMIN && request.getRole() != Role.ADMIN) {
            assertNotLastAdmin(user.getId());
        }
    ...
    // Counts active ADMINs excluding the one being changed; if that leaves zero, the
    // change would remove the last admin entirely, so it's blocked.
    private void assertNotLastAdmin(UUID excludingUserId) {
        long otherActiveAdmins = userRepository.findByDeletedFalse().stream()
                .filter(u -> u.getRole() == Role.ADMIN && !u.getId().equals(excludingUserId))
                .count();
        if (otherActiveAdmins == 0) {
            throw new BadRequestException("Cannot remove or demote the last remaining admin account");
        }
    }
