...
public class AllocationController {
    ...
    // NEW: the individually-barcoded units that made up this allocation, each with its
    // own downloadable barcode image - powers the "View Barcodes" table. ADMIN may view
    // any allocation; a DEPARTMENT caller only their own (enforced in the service).
    @GetMapping("/{id}/units")
    public ResponseEntity<List<InventoryUnitDTO>> units(@PathVariable UUID id) {
        User currentUser = userService.getCurrentUser();
        return ResponseEntity.ok(allocationService.getUnitsForAllocation(id, currentUser));
    }
